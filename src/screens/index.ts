import LoginScreen from './auth/login-screen/login-screen';
import Signup from './auth/signup-screen/signup-screen';
import Chatscreen from './main/chat-screen/chat-screen';
import HomeScreen from './main/home-screen/home-screen';
import InboxScreen from './main/inbox-screen/inbox-screen';
import PatientScreen from './main/patient-screen/patient-screen';
import ProfileScreen from './main/profile-screen/profile-screen';
import ForgetPassword from './auth/login-screen/fogetPassword';
import ContactScreen from './main/inbox-screen/all-contacts/contacts-screen';
import PracticeScreen from './main/practice/practice-screen';
import ViewScreen from './main/view-screen/view-screen';
export {
  LoginScreen,
  Signup,
  HomeScreen,
  InboxScreen,
  Chatscreen,
  ProfileScreen,
  PatientScreen,
  ForgetPassword,
  ContactScreen,
  PracticeScreen,
  ViewScreen,
};
